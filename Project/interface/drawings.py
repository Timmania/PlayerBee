...
def instructions_block(screen, center_y, center_x, h, w):
    ...
    screen.addstr(center_y + 8, x - 2, "{0:<52}".format(" - Het woord is minimaal 4 letters lang."))
    screen.addstr(center_y + 9, x - 2, "{0:<52}".format(" - De middelste letter moet er sowieso in zitten."))
    screen.addstr(center_y + 10, x - 2, "{0:<52}".format(" - Scheldwoorden leveren geen punten op."))
    screen.addstr(center_y + 11, x - 2, "{0:<52}".format(" - Dezelfde letter mag meerdere keren gebruikt worden."))
    screen.addstr(center_y + 13, x, "{0:^62}".format("Druk op een toets voor een uitleg van het speelscherm."))
    screen.addstr(center_y + 14, x, "{0:^62}".format("Druk op Esc om de uitleg te verlaten."))
    key = screen.getch()
    if key == 27:
        return ""
    y = (h - 6) // 2
    ...
    instructions = [
        ...
        ["hier vind je de moeilijkheisgraat", "van de huidige letters.", 5, (w // 4) + 24, "nw"],
        ["Hier vind je de letters", "waarmee jij woorden moet raden.", (h - 6) // 2 + 6, (w // 4) + 15, "nw"],
        ["Hier kun je tijdens het spel woorden typen.", h - 10, (w // 4) - 15, "se"],
        ["Hier komt de feedback van jouw antwoord.", h - 11, (w // 4) + 8, "se"],
        ["Hier staan jouw totale punten", "en de woorden die nog over zijn.", h - 9, (w // 4) * 3 - 32, "se"],
        ["Hier staan opties die jij kan typen", "tijdens het spel.", h - 12, (w // 4) * 3 - 29, "se"],
        ["Hier staan woorden die jij al gevonden hebt.", 6, (w // 4) * 3 - 32, "ne"]
    ]
    current_idx = 0
    while 1:
        textpad.rectangle(screen, y - 3, x - 24, y + 6, x + 24)
        screen.addstr(y - 2, x - 7, "{0} {1}/{2}".format("Instructies", current_idx + 1, len(instructions)))
        ...
        if len(instructions[current_idx]) > 4:
            screen.addstr(y + 1, x - 22, "{0:44}".format(instructions[current_idx][1]))
        screen.addstr(y + 3, x - 21, "{0:^44}".format("Navigeer met de pijltoetsen door de uitleg."))
        screen.addstr(y + 4, x - 23, "{0:^41}".format("Druk op Esc om de uitleg te verlaten."))
        if current_idx > 0:
            interface.give_feedback(screen, "PANGRAM FOUND! +14", h, w)
            arrow_screen = arrow(
                screen,
        ...
        if (key == cs.KEY_UP or key == cs.KEY_RIGHT) and current_idx < len(instructions) - 1:
            current_idx += 1
